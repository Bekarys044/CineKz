...
function MovieList({ generId, index_ }) {
  ...
  const getMovieByGenereId = () => {
    GlobalApi.getMovieByGenereId(generId)
      .then((resp) => {
  ...
  return (
    <div className="relative group">
      ...
      <div
        ref={elementRef}
        className="flex overflow-x-auto gap-4 scroll-smooth scrollbar-hide p-8 px-8 md:px-16 group"
      >
                     {
      ...
      <ArrowRight
        onClick={sliderRight}
        className="hidden md:block opacity-0 transition-opacity duration-300 group-hover:opacity-100 absolute top-[30%] md:top-[50%] right-[-30px] z-10 text-[var(--theme-color)] text-[65px] cursor-pointer"
      />
    </div>
  );
}
